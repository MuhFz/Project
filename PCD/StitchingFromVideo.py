# -*- coding: utf-8 -*-
"""
Created on Wed Nov  6 08:22:36 2024
...
class SurfStitcher:
    def __init__(self, image, ratio = 0.75, reprojThresh = 4.0):
        ...
        #self.surfFeature = cv2.xfeatures2d.SURF_create(500, extended = True, upright = True)
    ...
    def saveImage(self):
        cv2.imshow("Image", image_resize(self.leftImage, width = 900))
        cv2.waitKey()
...
def main():
    ...
    for i in range(3):
        ...
        cv2.imshow("Video", image_resize(frame, height = 500))
...
def showImage(image):
    cv2.imshow("Image", image_resize(image, height = 400))
    cv2.waitKey()
